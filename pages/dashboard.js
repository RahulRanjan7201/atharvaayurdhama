...
const Dashboard = ({ user, loading }) => {
  ...
  const createOption = (blogStatus) => {
    return blogStatus === "draft"
      ? { view: "Publish to Blog", value: "published" }
  ...
  const createOptions = (blog) => {
    ...
    return [
      ...
      {
        key: `${blog._id}-publshed`,
        text: "Publish to Telegram",
        handlers: {
          onClick: () => changeBlogStatus(blog._id, "telegram"),
        },
      },
      {
        key: `${blog._id}-publshed`,
        text: "Publish to Both",
        handlers: {
          onClick: () => changeBlogStatus(blog._id, "both"),
        },
      },
      {
        key: `${blog._id}-delete`,
